...
export function PixelViewer({ npyPath }: Props): JSX.Element {
  ...
  const applyRegionSelection = (region: Region) => {
    const start = clampPoint({ x: Math.min(region.x1, region.x2), y: Math.min(region.y1, region.y2) })
  ...
  const selectedRegionRect =
    selectedRegion && pixelScale
      ...
      : null
  const draggingRect = dragStart && dragEnd && pixelScale ? getOverlayRect(dragStart, dragEnd) : null
              ...
              <div className="max-h-40 overflow-y-auto text-xs text-zinc-600">
                {regionValues.map((stat) => (
                  <div key={stat.channel}>
                    Канал {stat.channel}: mean={stat.mean.toFixed(4)}, min={stat.min.toFixed(4)}, max={stat.max.toFixed(4)}
                  </div>
                ))}
              </div>
            </div>
